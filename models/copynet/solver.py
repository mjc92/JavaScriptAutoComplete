...
def autocomplete(args, batch, vocab, model, save_dir):
    ...
    while(1):
        ...
        inputs = Variable(torch.LongTensor(inputs_unk_np)).cuda()
        targets = Variable(torch.LongTensor(2,15).zero_()).cuda()
        # run model to get outputs
        encoded, _ = encoder(inputs)
        ...
        print("\n============================ Output ===========================\n", 
              ' '.join(vocab.idx_list_to_word_list(pred,idx2oov)))
        fix = 'temp'
        while(1):
            fix = input("\nFix output: ")
            if len(fix)==0:
                break
            large_dict = {**vocab.w2i, **oov2idx}
            fix = lex(fix,'value')
            fix_np = np.array(vocab.word_list_to_idx_list(fix,oov2idx),dtype=int)
            fix_var = Variable(torch.LongTensor(fix_np)).cuda()
            decoder_in, s, w = decoder_initial(inputs.size(0),vocab.w2i['<SOS>'])
            # targets[0,:len(fix_var)] = fix_var
            for j in range(args.out_seq):
                if j==0:
                    outputs,s,w = decoder(input_idx=decoder_in, encoded=encoded,
                    encoded_idx=inputs_oov_np,prev_state=s,
                    ...
                    if j<=len(fix_np):
                        # if the next input is a non-existing word
                        if fix[j-1] not in large_dict:
                            tmp_list1 = []
                            tmp_list2 = []
                            for k,v in large_dict.items():
                                if fix[j-1] in k:
                                    tmp_list1.append(v)
                                    tmp_list2.append(outputs[0,-1,v].data[0])
                            tmp_lst1 = np.array(tmp_list1)
                            tmp_list2 = np.array(tmp_list2)
                            max_val = tmp_lst1[tmp_list2.argmax()]
                            print(max_val)
                            print(targets[0,j-1])
                            targets[0,j-1] = Variable(torch.LongTensor(max_val)).cuda()
                        decoder_in = targets[:,j-1]
                    else:
                        decoder_in = outputs[:,-1,:].max(1)[1].squeeze()
                    tmp_out,s,w = decoder(input_idx=decoder_in, encoded=encoded,
                        encoded_idx=inputs_oov_np, prev_state=s,
